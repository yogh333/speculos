...
class TextWidget(FrameBuffer):
    def __init__(self, parent, model):
        ...
        if os.tcgetpgrp(sys.stdin.fileno()) != os.getpgrp():
            logging.getLogger("display").warn(
                "please run speculos in the foreground to allow the initialization of the display"
            )
    ...
    def _redraw(self):
        ...
        for i in range(0, self.height - 2, 2):
            ...
            self.stdscr.addstr(
                1 + i // 2, self.width // 2 + 1, " ", curses.color_pair(2)
            )

        self.stdscr.addstr(0, 0, " " * (self.width // 2 + 2), curses.color_pair(2))
        self.stdscr.addstr(
            self.height // 2, 0, " " * (self.width // 2 + 2), curses.color_pair(2)
        )
        self.stdscr.refresh()
...
def display(
    apdu, seph, button_tcp=None, finger_tcp=None, model="nanos", keymap=None, **_
):
