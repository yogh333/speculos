...
def get_elf_infos(app_path):
    with open(app_path, "rb") as fp:
        ...
        ram_addr, ram_size = (
            (supp_ram["sh_addr"], supp_ram["sh_size"])
            if supp_ram is not None
            else (0, 0)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--debug", action="store_true", help="Wait gdb connection to port 1234"
    )
    parser.add_argument(
        "--deterministic-rng",
        default="",
    ...
    parser.add_argument(
        ...
        default=[],
        action="append",
        help="Additional library (eg. Bitcoin:app/btc.elf) which can be called through os_lib_call",
    )
    parser.add_argument(
        "--log-level",
        default=[],
    ...
    parser.add_argument(
        "-m", "--model", default="nanos", choices=list(display.MODELS.keys())
    )
    parser.add_argument(
        "-r",
        "--rampage",
        help="Additional RAM page and size available to the app (eg. 0x123000:0x100). Supercedes the internal probing for such page.",
    )
    parser.add_argument(
        "-s",
        "--seed",
    ...
    group.add_argument(
        "--apdu-port", default=9999, type=int, help="ApduServer TCP port"
    )
    group.add_argument(
        "--automation-port",
        type=int,
    ...
    group.add_argument(
        "--vnc-port", type=int, help="Start a VNC server on the specified port"
    )
    group.add_argument(
        "--vnc-password",
        type=str,
    ...
    group.add_argument(
        "--keymap",
        action="store",
        help="Text UI keymap in the form of a string (e.g. 'was' => 'w' for left button, 'a' right, 's' both). Default: arrow keys",
    )
    group.add_argument(
        "--progressive",
        action="store_true",
    ...
    group.add_argument(
        "--zoom", help="Display pixel size.", type=int, choices=range(1, 11)
    )
    ...
    if args.automation_port:
        ...
        automation_server = AutomationServer(
            ("0.0.0.0", args.automation_port), AutomationClient
        )
    ...
    seph = seproxyhal.SeProxyHal(
        s2, automation=automation_path, automation_server=automation_server
    )
