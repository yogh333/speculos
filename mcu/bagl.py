...
class Bagl:
    ...
    def hal_draw_bitmap_within_rect(
        self, x, y, width, height, colors, bpp, bitmap, restore=None
    ):
        if bpp == 3 or bpp > 4:
            return
        ...
        while bitmap:
            ...
            for i in range(0, 8, bpp):
                if (
                    xx >= 0
                    and xx < self.SCREEN_WIDTH
                    and yy >= 0
                    and yy < self.SCREEN_HEIGHT
                ):
                ...
                if xx >= requested_xw:
                    ...
                    if yy >= requested_yh:
                        self.draw_state = DrawState(
                            x, y, width, height, colors, bpp, xx, yy
                        )
                        return
    ...
    def draw_string(
        self, font_id, fgcolor, bgcolor, x, y, width, height, text, text_encoding
    ):
        font = bagl_font.get(font_id)
        if not font:
            self.logger.error(
                "unsupported font {}".format(font_id & bagl_font.BAGL_FONT_ID_MASK)
            )
            return 0
        ...
        if font.bpp > 1:
            ...
            for off in range(0, 3):
                ...
                if cfg > cbg:
                    ...
                    for i in range(1, color_count):
                        colors[i] |= min(0xFF, cfg + (color_count - i) * cinc) << (
                            off * 8
                        )
        ...
        for ch in text:
            ...
            if ch < font.first_char or ch > font.last_char:
                ...
                if ch >= 0xC0:
                    ...
                    font_symbols = bagl_font.get(font_symbol_id)
                    if font_symbols:
                        ch_bitmap = font.bitmap[
                            font.characters[ch & 0x1F].bitmap_offset :
                        ]
                        ch_width = font.characters[ch & 0x1F].char_width
            ...
            if ch_bitmap:
                self.hal_draw_bitmap_within_rect(
                    xx, ch_y, ch_width, ch_height, colors, font.bpp, ch_bitmap
                )
            else:
    ...
    def _draw_circle_helper(
        self, color, x_center, y_center, radius, octants, radiusint, colorint
    ):
        x, y = radius, 0
        decisionOver2 = 1 - x
        ...
        while y <= x:
            if octants & 1:
                if drawint:
                    self.hal_draw_rect(
                        colorint, x_center, y + y_center, x - (dradius - 1), 1
                    )
                    self.hal_draw_rect(
                        color, x_center + x - (dradius - 1), y + y_center, dradius, 1
                    )
                else:
            ...
            if octants & 2:
                if drawint:
                    if last_x != x:
                        self.hal_draw_rect(
                            colorint, x_center, x + y_center, y - (dradius - 1), 1
                        )
                    self.hal_draw_rect(
                        color, x_center + y - (dradius - 1), x + y_center, dradius, 1
                    )
                else:
            ...
            if octants & 4:
                if drawint:
                    self.hal_draw_rect(
                        colorint, x_center - x, y + y_center, x - (dradius - 1), 1
                    )
                    self.hal_draw_rect(
                        color, x_center - x - (dradius - 1), y + y_center, dradius, 1
                    )
            ...
            if octants & 8:
                if drawint:
                    if last_x != x:
                        self.hal_draw_rect(
                            colorint, x_center - y, x + y_center, y - (dradius - 1), 1
                        )
                    self.hal_draw_rect(
                        color, x_center - y - (dradius - 1), x + y_center, dradius, 1
                    )
            ...
            if octants & 16:
                if drawint:
                    self.hal_draw_rect(
                        colorint, x_center, y_center - y, x - (dradius - 1), 1
                    )
                    self.hal_draw_rect(
                        color, x_center + x - (dradius - 1), y_center - y, dradius, 1
                    )
                else:
            ...
            if octants & 32:
                if drawint:
                    if last_x != x:
                        self.hal_draw_rect(
                            colorint, x_center, y_center - x, y - (dradius - 1), 1
                        )
                    self.hal_draw_rect(
                        color, x_center + y - (dradius - 1), y_center - x, dradius, 1
                    )
                else:
            ...
            if octants & 64:
                if drawint:
                    self.hal_draw_rect(
                        colorint, x_center - x, y_center - y, x - (dradius - 1), 1
                    )
                    self.hal_draw_rect(
                        color, x_center - x - (dradius - 1), y_center - y, dradius, 1
                    )
            ...
            if octants & 128:
                if drawint:
                    if last_x != x:
                        self.hal_draw_rect(
                            colorint, x_center - y, y_center - x, y - (dradius - 1), 1
                        )
                    self.hal_draw_rect(
                        color, x_center - y - (dradius - 1), y_center - x, dradius, 1
                    )
    ...
    def _display_bagl_icon(self, component, context):
        if component.icon_id != 0:
            ...
            if len(context) != 0:
                ...
                for i in range(0, 1 << glyph.bpp):
                    color = int.from_bytes(
                        context[i * 4 : (i + 1) * 4], byteorder="big"
                    )
                    colors.append(color)
            else:
                colors = glyph.colors
            ...
            if len(context) == 0:
                self.logger.info(
                    "len context == 0 {}".format(binascii.hexlify(context))
                )
                return
    ...
    def _display_bagl_rectangle(
        self, component, context, context_encoding, halignment, valignment
    ):
        ...
        if context:
            ...
            self.draw_string(
                ...
                component.width
                - halignment
                - stroke,  # XXX: take icon_width into account
    ...
    def _display_get_alignment(self, component, context, context_encoding):
        ...
        if font:
            ...
            if context:
                strwidth = Bagl.compute_line_width(
                    component.font_id, component.width + 100, context, context_encoding
                )
    ...
    def display_status(self, data):
        ...
        if type_ == BAGL_NONE:
            ...
            self._display_bagl_rectangle(
                component, context, context_encoding, halignment, valignment
            )
